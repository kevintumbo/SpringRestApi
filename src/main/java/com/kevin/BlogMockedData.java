...
public class BlogMockedData {
    ...
    public List<Blog> searchBlogs(String searchTerms) {
        List<Blog> searchedBlogs = new ArrayList<Blog>();
        for (Blog b: blogs) {
            if(b.getTitle().toLowerCase().contains(searchTerms.toLowerCase()) ||
                    b.getContent().toLowerCase().contains(searchTerms.toLowerCase())) {
                searchedBlogs.add(b);
            }
        }
